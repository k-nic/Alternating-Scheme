...
def gaussian_kernel(X):
    ...
    col = np.reshape(np.sum(X*X,axis=1)/dim, (-1,1))
    row = np.transpose(col)
    return np.exp(2*Gram-col-row)
...
def laplace_kernel(X):
    ...
    col = np.reshape(np.sum(X*X,axis=1)/dim, (-1,1))
    row = np.transpose(col)
    return np.exp(-5*np.sqrt(col+row-2.0*Gram+0.0001))
...
def poisson_kernel(X):
    ...
    col = np.reshape(np.sum(X*X,axis=1)/dim, (-1,1))
    row = np.transpose(col)
    return np.power(1.0+0.04*(col+row-2.0*Gram), -(dim+1)/2)
...
def gaussian_median(video_path):
    ...
    X = np.reshape(Frames, (-1,dim))
    Y = kernel_calc(X,gaussian_kernel,n_components=1)
    Y = np.reshape(Y,Frames.shape[0:4])
    Y = np.median(Y, axis=0)
    Y = np.around(Y)
    Y = Y.astype('uint8')
...
def laplace_median(video_path):
    ...
    X = np.reshape(Frames, (-1,dim))
    Y = kernel_calc(X,laplace_kernel,n_components=5)
    Y = np.reshape(Y,Frames.shape[0:4])
    Y = np.median(Y, axis=0)
    Y = np.around(Y)
    Y = Y.astype('uint8')
...
def poisson_median(video_path):
    ...
    X = np.reshape(Frames, (-1,dim))
    Y = kernel_calc(X,poisson_kernel,n_components=5)
    Y = np.reshape(Y,Frames.shape[0:4])
    Y = np.median(Y, axis=0)
    Y = np.around(Y)
    Y = Y.astype('uint8')
